            ...
            return {
                workspaces: state.workspaces.map((workspace) => {
                    ...
                    return {
                        ...
                        ideas: workspace.ideas.map((idea) => {
                            if (idea.id !== ideaId) return idea;
                            // A rename is the moment a title becomes chosen — it
                            // earns the serif from here on.
                            if (idea.kind === "project") {
                                return { ...idea, title: trimmedTitle, isTitleAutoGenerated: false };
                            }
                            ...
                            return {
                                ...
                                clips: idea.clips.map((clip) =>
                                    clip.id === primaryClipId
                                        ? { ...clip, title: trimmedTitle, isTitleAutoGenerated: false }
                                        : clip
                                ),
                            };
                        }),
