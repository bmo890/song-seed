...
export function useRecordingScreenModel() {
  ...
  async function saveQuickClipName() {
    ...
    // A typed (or accepted) name is a name; an empty sheet keeps the machine title.
    const isTitleAutoGenerated = quickNameDraft.trim().length === 0;
        ...
        if (idea.kind === "clip") {
          return {
            ...
            clips: idea.clips.map((clip) =>
              clip.id === firstClipId ? { ...clip, title: nextTitle, isTitleAutoGenerated } : clip
            ),
          };
        }
