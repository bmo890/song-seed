...
export const ClipCard = React.memo(function ClipCard({
  ...
  const durationLabel = playbackDurationMs ? fmtCardDuration(playbackDurationMs) : "0:00";
  const inlineTotalMs = inlineDurationMs || playbackDurationMs || 0;
  // Auto-named clips carry the absolute time in their title, so the meta row
  // shows relative recency instead of echoing it; named clips keep time-of-day.
  const titleIsAuto = isClipTitleAutoNamed(clip);
  const createdAtTime = titleIsAuto
    ? formatClipDate(clip.createdAt)
    : formatClipCardTime(clip.createdAt);
  const createdAtWithLayers =
    overdubStemCount > 0
      ? `${createdAtTime} • ${t("navigation.layerCount", { count: overdubStemCount })}`
      : createdAtTime;
  // Thread heads carry their version quietly in the footer: "v2 · Just now".
  const versionPrefix =
    entry.kind === "evolution" && !entry.indented && (entry.versionCount ?? 0) > 1
  ...
  return (
    <View style={styles.songDetailClipRowWrap}>
      ...
      <IdeaCard
        ...
        // The waveform strip is the inline player: progress + scrub map straight
        // onto the existing inline preview state — no engine changes. displayOnly
        // cards keep their previous behavior (no inline controls surface).
        inlineProgress={
