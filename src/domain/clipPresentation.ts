 ...
 *
 * A filename is not a chosen name — an imported "sample-01" was named by
 * whatever produced the file, so it stays in quiet tabular Jakarta until the
 * musician actually names it. Renaming sets `isTitleAutoGenerated: false`
 * explicitly, so legacy imports (flag undefined + `importedAt` present) are
 * still correctly read as auto-named without a data migration.
 */
export function isClipTitleAutoNamed(clip: ClipVersion): boolean {
  if (clip.isTitleAutoGenerated === true) return true;
  if (clip.isTitleAutoGenerated === undefined && clip.importedAt) return true;
  return false;
}
