...
import { isTitleAutoNamed } from "./titleNaming";
...
/**
 * Does this clip's title earn the serif? (design-system §earned serif)
 * One rule for ideas and clips — see `domain/titleNaming.ts`.
 */
export function isClipTitleAutoNamed(clip: ClipVersion): boolean {
  return isTitleAutoNamed(clip);
}
