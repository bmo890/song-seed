...
const STANDALONE_TAKE2_AUDIO = "file:///doc/songnook/audio/standalone-2.m4a";
const CHILD2_CLIP_AUDIO = "file:///doc/songnook/audio/child-2.m4a";
const STEM_AUDIO = "file:///doc/songnook/audio/stem.m4a";
...
function buildLibrary(): { workspaces: Workspace[]; idea: SongIdea } {
    const idea: SongIdea = {
        ...
        clips: [
            ...
            },
            {
                ...
                // Editor "save as new clip": the pre-edit original is the root's file.
                sourceAudioUri: ROOT_CLIP_AUDIO,
                durationMs: 11000,
    ...
    const standalone: SongIdea = {
        ...
        clips: [
            ...
            // A re-record on the clip idea: the recorder appends takes to clip ideas too.
            {
                id: "clip-standalone-2",
                title: "Second go",
                notes: "second take notes",
                createdAt: 450,
                isPrimary: false,
                isBookmarked: true,
                parentClipId: "clip-standalone",
                audioUri: STANDALONE_TAKE2_AUDIO,
                durationMs: 5200,
                waveformPeaks: [0.7],
            },
        ],
            ...
            collections: [
                {
                    ...
                    updatedAt: 20,
                    ideasListState: {
                        hiddenIdeaIds: ["idea-2"],
                        hiddenDays: [{ metric: "created", dayStartTs: 86400000 }],
                    },
                },
            ],
            ideas: [idea, standalone],
            ideasListState: { hiddenIdeaIds: ["idea-1"], hiddenDays: [] },
        },
    ];
...
beforeEach(() => {
    ...
    mockFiles.set(STANDALONE_TAKE2_AUDIO, utf8("standalone-take-2-bytes"));
    mockFiles.set(CHILD2_CLIP_AUDIO, utf8("child-2-audio-bytes"));
    mockFiles.set(STEM_AUDIO, utf8("stem-audio-bytes"));
});

async function exportThenImport(preserveAllMetadata: boolean, overrides?: { includeLyrics?: boolean }) {
    const { workspaces } = buildLibrary();
    const prepared = await prepareLibraryExportArchive({
        workspaces,
        notes: [
            { id: "note-1", title: "שיר", body: "מילים", createdAt: 1, updatedAt: 2, isPinned: true, textDirection: "rtl" },
        ],
        ...
        options: {
            ...
            includeNotes: true,
            includeLyrics: overrides?.includeLyrics ?? true,
...
describe("SongNook Archive round-trip — full fidelity", () => {
    ...
    it("preserves workspace, collection and project metadata", async () => {
        ...
        expect(song.isTitleAutoGenerated).toBe(true);
    ...
    it("keeps hidden-idea state on the collection and workspace, remapped to the new ids", async () => {
        const { workspace } = await exportThenImport(true);
        const song = workspace.ideas.find((idea) => idea.kind === "project")!;
        const standalone = workspace.ideas.find((idea) => idea.kind === "clip")!;
        expect(workspace.collections[0].ideasListState).toEqual({
            hiddenIdeaIds: [standalone.id],
            hiddenDays: [{ metric: "created", dayStartTs: 86400000 }],
        });
        expect(workspace.ideasListState?.hiddenIdeaIds).toEqual([song.id]);
    });

    it("keeps a note's text direction", async () => {
        const { merge } = await exportThenImport(true);
        expect(merge.importedNotes).toHaveLength(1);
        expect(merge.importedNotes[0]).toMatchObject({ title: "שיר", textDirection: "rtl", isPinned: true });
    });

    it("keeps overdub layer colour and creation time", async () => {
        const { workspace } = await exportThenImport(true);
        const song = workspace.ideas.find((idea) => idea.kind === "project")!;
        const root = song.clips.find((clip) => clip.isPrimary)!;
        expect(root.overdub?.stems).toHaveLength(1);
        expect(root.overdub?.stems[0]).toMatchObject({ title: "Harmony", color: "#123456", createdAt: 777, gainDb: -3 });
        expect(root.overdub?.stems[0].audioUri).toMatch(/^file:\/\/\/doc\/songnook\/audio\//);
    });

    it("packs the pre-edit source once and shares it between derived clips", async () => {
        const { parsed, workspace } = await exportThenImport(true);
        const sourceEntries = Array.from(parsed.archiveIndex.entries.keys()).filter((name) => / Source\.m4a$/.test(name));
        expect(sourceEntries).toHaveLength(1);

        const song = workspace.ideas.find((idea) => idea.kind === "project")!;
        const edits = song.clips.filter((clip) => clip.title.startsWith("Edit"));
        expect(edits).toHaveLength(2);
        expect(edits[0].sourceAudioUri).toBeDefined();
        expect(edits[0].sourceAudioUri).not.toBe(edits[0].audioUri);
        expect(edits[1].sourceAudioUri).toBe(edits[0].sourceAudioUri);
        expect(mockFiles.get(edits[0].sourceAudioUri!)).toEqual(utf8("root-audio-bytes"));
        // The root itself has no separate source, so none is invented.
        expect(song.clips.find((clip) => clip.isPrimary)!.sourceAudioUri).toBeUndefined();
    });

    it("keeps the take's lyric version only when lyrics travel", async () => {
        const withLyrics = await exportThenImport(true);
        const rootWith = withLyrics.workspace.ideas.find((i) => i.kind === "project")!.clips.find((c) => c.isPrimary)!;
        expect(rootWith.lyricsVersionId).toBe("lv-2");

        const without = await exportThenImport(true, { includeLyrics: false });
        const rootWithout = without.workspace.ideas.find((i) => i.kind === "project")!.clips.find((c) => c.isPrimary)!;
        expect(rootWithout.lyricsVersionId).toBeUndefined();
    });
    ...
    it("preserves standalone clip metadata", async () => {
        const { workspace } = await exportThenImport(true);
        const standalone = workspace.ideas.find((idea) => idea.kind === "clip")!;
        const clip = standalone.clips.find((candidate) => candidate.isPrimary)!;
    ...
    it("keeps every take of a clip idea, with lineage, plus the idea's own fields", async () => {
        const { parsed, workspace } = await exportThenImport(true);
        const standalone = workspace.ideas.find((idea) => idea.kind === "clip")!;
        expect(standalone.clips).toHaveLength(2);
        const primary = standalone.clips.find((clip) => clip.isPrimary)!;
        const second = standalone.clips.find((clip) => !clip.isPrimary)!;
        expect(primary.title).toBe("Loose clip");
        expect(second).toMatchObject({ title: "Second go", notes: "second take notes", isBookmarked: true, durationMs: 5200 });
        expect(second.parentClipId).toBe(primary.id);
        expect(mockFiles.get(second.audioUri!)).toEqual(utf8("standalone-take-2-bytes"));
        expect(mockFiles.get(primary.audioUri!)).toEqual(utf8("standalone-audio-bytes"));

        expect(standalone.notes).toBe("idea notes");
        expect(standalone.isTitleAutoGenerated).toBe(true);
        expect(standalone.isDraft).toBe(true);
        expect(standalone.lastActivityAt).toBe(450);

        // The primary take's audio is packed once, not once per representation.
        const standaloneEntries = Array.from(parsed.archiveIndex.entries.keys()).filter((name) => name.includes("Loose clip"));
        expect(standaloneEntries.filter((name) => name.endsWith("Loose clip.m4a"))).toHaveLength(1);
        expect(standaloneEntries.some((name) => name.includes("Loose clip Takes/Second go.m4a"))).toBe(true);
    });
...
describe("SongNook Archive round-trip — standard (lossy)", () => {
    it("drops creative metadata and regenerates the waveform", async () => {
        ...
        // Edit regions travel in ALL fidelities: the packed audio is the untouched master,
        // so without them a recipient would hear the raw, untrimmed take.
        expect(root.editRegions).toEqual([{ id: "er-1", startMs: 1000, endMs: 2000, type: "remove" }]);
        // Standard archives keep only the primary take of a clip idea.
        const standalone = workspace.ideas.find((idea) => idea.kind === "clip")!;
        expect(standalone.clips).toHaveLength(1);
