...
import {
    ...
    type ArchiveStandaloneClipManifest,
...
type ExportBuildContext = {
    ...
    warnings: string[];
    /** Pre-edit source audio already packed, by live file URI → archive path (dedupe). */
    sourceAudioPaths: Map<string, string>;
    exportedWorkspaces: number;
    exportedCollections: number;
    exportedSongs: number;
    exportedStandaloneClips: number;
...
export async function prepareLibraryExportArchive(args: ExportLibraryArgs): Promise<LibraryExportResult> {
    ...
    await ensureBackupDiskSpace(statedSourceBytes, "export");
        ...
        const workspaceManifest: ArchiveWorkspaceManifest = {
            ...
            ...(preserveAllMetadata
                ? {
                      ...
                      ideasListState: workspace.ideasListState,
                  }
                : null),
        };
 ...
 * `lyricsVersionId` only travels when the lyric versions do — otherwise it would dangle.
 */
function applyFullClipMetadata(
    manifest: ArchiveClipManifest,
    clip: ClipVersion,
    options: SongNookArchiveOptions
): void {
    ...
    manifest.lyricsVersionId = options.includeLyrics ? clip.lyricsVersionId : undefined;
    manifest.manualSortOrder = clip.manualSortOrder;
}

/**
 * Pack the pre-edit original alongside the master (full fidelity). Clips derived from the
 * same source (editor "save as new clip") all point at one file, so it is written once and
 * shared by archive path.
 */
function attachSourceAudio(
    ...
    const sourceUri = clip.sourceAudioUri;
    if (!sourceUri || !clip.audioUri || sourceUri === clip.audioUri) return;
    const existing = context.sourceAudioPaths.get(sourceUri);
    if (existing) {
        manifest.sourceAudioPath = existing;
        return;
    }
    const sourcePath = `${clipStemPath} Source.${getArchiveFileExtension(sourceUri)}`;
    context.sourceAudioPaths.set(sourceUri, sourcePath);
    manifest.sourceAudioPath = sourcePath;
    addFileEntry(context, sourcePath, sourceUri, `Missing source audio for ${clipLabel}.`);
}
...
function applyFullSongMetadata(manifest: ArchiveSongManifest, idea: SongIdea): void {
    ...
    manifest.isTitleAutoGenerated = idea.isTitleAutoGenerated;
...
/**
 * One take of an idea: the master audio, notes, overdubs and (full fidelity) every
 * creative field. Shared by song takes and clip-idea takes so the two can never drift.
 */
function buildArchiveClipEntry(
    clip: ClipVersion,
    clipStemPath: string,
    options: SongNookArchiveOptions,
    context: ExportBuildContext,
    clipLabel: string,
    behaviour?: { writeNotesText?: boolean }
): ArchiveClipManifest {
    ...
    if (clipAudioUri) {
        ...
        addFileEntry(context, audioPath, clipAudioUri, `Missing clip audio for ${clipLabel}.`);
    } else {
        clipManifest.audioMissing = true;
        context.warnings.push(`Missing clip audio for ${clipLabel}.`);
    }

    const overdubManifest = buildArchiveClipOverdubManifest(clip, clipStemPath, context, clipLabel, options);
    if (overdubManifest) {
        clipManifest.overdub = overdubManifest;
    }

    if (options.preserveAllMetadata) {
        applyFullClipMetadata(clipManifest, clip, options);
        clipManifest.isBookmarked = clip.isBookmarked;
        attachSourceAudio(clipManifest, clip, clipStemPath, context, clipLabel);
    }
...
function buildArchiveSongManifest(
    ...
    clipsToExport.forEach((clip) => {
        ...
        songManifest.clips.push(
            buildArchiveClipEntry(clip, clipStemPath, options, context, `${idea.title} / ${clip.title || clip.id}`)
        );
    });
...
function buildArchiveStandaloneClip(
    ...
    options: SongNookArchiveOptions,
    context: ExportBuildContext
): ArchiveStandaloneClipManifest {
    const primaryClip = getPrimaryClip(idea);
    const clipStemPath = allocateChildPath(collectionFolderPath, idea.title, itemAllocator, "Clip");
    // The primary take is packed once, at the idea's own stem path. Its notes text file is
    // skipped here because the entry writes the IDEA's notes at that path.
    const primaryTake = primaryClip
        ? buildArchiveClipEntry(primaryClip, clipStemPath, options, context, idea.title, {
              writeNotesText: false,
          })
        : null;
    if (!primaryTake) {
    ...
    // The entry describes the PRIMARY take under the idea's identity (title, bookmark,
    // notes), so pre-v7 importers keep reading it exactly as before.
    const clipManifest: ArchiveStandaloneClipManifest = {
        ...(primaryTake ?? { audioMissing: true }),
        id: idea.id,
        title: idea.title,
        createdAt: idea.createdAt,
        isPrimary: true,
    ...
    // Full fidelity: a clip idea keeps every take (re-records land here too), each with
    // its own notes, bookmark and lineage. The primary take reuses the entry's audio file;
    // the rest live in a "<idea> Takes" folder.
    const takesToExport = options.includeFullSongHistory ? idea.clips : primaryClip ? [primaryClip] : [];
    const takeAllocator = createFolderAllocator();
    const takesFolderPath = `${clipStemPath} Takes`;
    clipManifest.takes = takesToExport.map((clip) => {
        if (clip === primaryClip && primaryTake) {
            return primaryTake;
        }
        const takeStemPath = allocateChildPath(takesFolderPath, clip.title || idea.title, takeAllocator, "Take");
        return buildArchiveClipEntry(clip, takeStemPath, options, context, `${idea.title} / ${clip.title || clip.id}`);
    });
    clipManifest.idea = {
        status: idea.status,
        completionPct: idea.completionPct,
        lastActivityAt: idea.lastActivityAt,
        isTitleAutoGenerated: idea.isTitleAutoGenerated,
        isDraft: idea.isDraft,
        importedAt: idea.importedAt,
        sourceCreatedAt: idea.sourceCreatedAt,
...
function createExportBuildContext(): ExportBuildContext {
    return {
        ...
        entries: [],
        warnings: [],
        sourceAudioPaths: new Map<string, string>(),
...
function buildArchiveClipOverdubManifest(
    ...
    stems.forEach((stem) => {
        const stemManifest: ArchiveClipOverdubStemManifest = {
            ...
            ...(options.preserveAllMetadata ? { color: stem.color, createdAt: stem.createdAt } : null),
        };
