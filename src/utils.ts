...
/**
 * Does an idea's title earn the serif? (design-system §earned serif)
 *
 * `isTitleAutoGenerated` is authoritative once set — a rename writes `false`.
 * Older records predate the flag, so fall back to the two machine-supplied
 * shapes: an unedited auto timestamp, or a filename an import arrived with.
 */
export function isIdeaTitleAutoNamed(idea: {
  title: string;
  createdAt: number;
  importedAt?: number;
  isTitleAutoGenerated?: boolean;
}) {
  if (idea.isTitleAutoGenerated !== undefined) return idea.isTitleAutoGenerated;
  if (isDefaultIdeaTitle(idea.title, idea.createdAt)) return true;
  return !!idea.importedAt;
}
