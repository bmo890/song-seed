...
"SongNook" is two common words already used by several apps, and there's a (currently inactive) `songnook` domain in someone else's hands. That raises three *separate* concerns — an App Store uniqueness blocker, a trademark question, and a discoverability drag — and at least the first can force a rename whether we like it or not. The decision has to be made and locked before Phase 0 because:
...
1. **App Store + Play Store search** for "SongNook" and close variants. *If a live app already holds "SongNook" on the App Store, the name is unusable* — Apple enforces unique app names. This single check may settle the whole question.
2. **USPTO TESS** (free, uspto.gov) for `songnook` / `songnook`. Filter to **live** marks in relevant classes: **Class 9** (software) and **Class 41** (entertainment/education services). A live registered mark for music software/services under this name is the real risk signal.
3. **Domain**: note that the inactive `songnook` domain confers *no* trademark rights on its own (rights come from use in commerce, not domain registration) — don't let it drive the decision, but a clean available `.com`/`.app` for the *new* name is a nice-to-have.
...
Critically, **most of the 488 `songnook` string hits in the repo are internal identifiers that should NOT change** — changing them is pure churn and risk with zero user or legal benefit:

### Leave alone (invisible internal identifiers)
- `modules/songnook-file-io`, `modules/songnook-metronome`, `modules/songnook-pitch-shift` — native module package names. Internal. Renaming means rewriting Swift/Kotlin package paths and podspecs for no user benefit.
- `SONGNOOK_UPLOAD_STORE_FILE` / `_PASSWORD` / `_KEY_ALIAS` gradle signing properties (`android/app/build.gradle`, `plugins/withSongNookAndroidVariants.js`) — build-time secret names.
- `songNookScheme` manifest-placeholder **variable name** and the `withSongNookAndroidVariants.js` **filename** — internal wiring (the placeholder's *value* is the scheme, which does change; the variable name doesn't need to).
- **Android Java namespace** `namespace 'com.bmostudio.songnook'` in `android/app/build.gradle` — this is the R-class package, separable from the store identity. The `app.config.js` comment already notes native generation "always uses the stable Java namespace." Keep it stable to avoid churn.

### The bundle ID / applicationId — a deliberate choice (read carefully)
The store-facing identity is `com.bmostudio.songnook` (iOS `bundleIdentifier`, Android `applicationId`). It is **invisible to users** — only visible to someone inspecting the app package. Two valid options:
- **Keep it** (`com.bmostudio.songnook`) even under a new display name. Totally normal after rebrands; the "songnook" token in a reverse-domain technical ID is not brand "use in commerce" and carries negligible trademark risk. **Zero churn, and nothing locks you out later** because the display name (which *is* the brand) is freely changeable forever.
...
**Recommendation:** unless the new brand really needs a matching bundle ID, **keep `com.bmostudio.songnook`** — it's the lowest-risk path and costs nothing. The brand lives in the display name, not the bundle ID.
...
| Deep-link scheme (+ `-dev`) | `app.json` L6, `app.config.js` L8; Android manifest placeholder **values** in `android/app/build.gradle` L95/L112 and `plugins/withSongNookAndroidVariants.js`; fallback literal in `App.tsx` L709 (`?? "songnook"`) |
| iOS share-extension name | `app.json` L50 (`iosShareExtensionName: "SongNook Import"`) |
| npm package name + scripts | `package.json` L2 (`name`), and the `songnook-dev` scheme flag in `start:dev` (L7) — the `com.bmostudio.songnook*` app-ids in scripts L9–11 only change if you change the bundle ID |
| Android launcher label | `android/app/src/main/res/values/strings.xml` `app_name` (currently stale `song-nook` — Phase 0.8 fixes this anyway; align to final name here) |
| iOS display name | `ios/songnook/Info.plist` `CFBundleDisplayName` |
| In-app brand wordmark | `src/components/SideNav.tsx` L98 (`<Text>SongNook</Text>`) |
| Permission usage strings | `app.json` L19/L56/L63/L72/L73 ("SongNook needs microphone access…", notification string) |
| In-app copy referencing the name | ~68 occurrences of "SongNook" in `src/**` (error messages, `restoreRuntime.ts`, `importDates.ts`, DB comments are fine to leave). **Centralize** into one `APP_NAME` constant (suggest `src/appMeta.ts`) and reference it, so the *next* rename is one line — most of these are in `SettingsScreen` backup/export copy. |
| The **"SongNook Archive"** file-format name | Same 68-site sweep — decide whether the backup/export format keeps "SongNook Archive" or adopts the new brand. It's a user-facing product term but legally decoupled from the app name; recommend renaming for consistency. **Note:** this is a display string only — do **not** change any archive *file magic / manifest identifiers / format detection strings* in `src/services/libraryArchiveManifest.ts` / `archiveKind.ts` etc., or existing users' backups stop being recognized. Verify the detection logic keys off a stable internal token, not the display name, before touching anything here. |
...
## Brand metaphor & voice (working candidate: "SongNook")

The leading rename candidate is **SongNook**. Whoever builds the icon (Phase 0.1) and writes store/onboarding copy (Phases 4–5) should inherit this thinking:

- **What it means:** the `-stead` suffix is Old English for *place / dwelling*, surviving mainly in **homestead** — a plot of land you settle, tend, and grow things on over time. "SongNook" = *a homestead for your songs*: warm, rooted, editorial, a little old-world.
- **Why it fits the existing "seed" language — it extends it, doesn't replace it.** The metaphor is one arc: **seed → planted → grows on the stead you tend → season after season.** "SongNook" named only the capture moment; the app's real depth is *tending* ideas over time (revisit, versions, practice loops, overdub layers, the archive/backup machinery). "SongNook" names the whole cultivated place — a truer promise for what the product does.
...
- **2026-07-11 — Working candidate:** **"SongNook"** chosen as the leading rename direction (see Brand metaphor & voice above). Rationale: more distinctive/ownable than the two-common-words "SongNook," and it upgrades the metaphor from capture-moment (seed) to the whole tended place (homestead), which matches the app's depth.
- **2026-07-11 — Name decision: FINAL — "SongNook."** Owner locked the name ("we are going with songnook 100%") and waived the clearance gate; Phase 0 proceeds against it. (A store/USPTO sanity search remains prudent before first submission, but no longer blocks work.)
- **2026-07-11 — Bundle ID decision: keep `com.bmostudio.songnook`.** Per the recommendation above — invisible to users, zero churn, negligible risk; the brand lives in the display name. Can still be revisited any time before first store submission (Phase 5), never after.
