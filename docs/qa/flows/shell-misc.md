...
| SHELL-36 | Meter change | Select meter chips (2/4 … 12/8, two rows) while running | Meter label + beat ring beats-per-bar update (dense dots above 8 pulses); downbeat accent lands on beat 1 (ear check); 7/8 shows its three groupings | PARTIAL | iOS+Android | P2 |
| SHELL-36b | Subdivision + sound | Tempo → Subdivision Halves/Thirds/Quarters; Cues → Sound Wood | Sub-clicks between beats at a quieter level, dots/haptic unchanged; wood voice lower and softer; controls absent on binaries without click-style support | PARTIAL | iOS+Android | P2 |
| SHELL-36c | Tempo marking + keep-awake | Drag tempo across 60/76/108/120/168/200; start and wait past auto-lock | Marking word under BPM changes at each band; screen stays awake only while running | PARTIAL | iOS+Android | P3 |
...
| SHELL-39 | Metronome persistence | Set BPM/meter (e.g. 7/8 + 3+2+2)/subdivision/sound/cues/levels → kill app → relaunch → Metronome | All settings restored from store | AUTO | iOS+Android | P3 |
