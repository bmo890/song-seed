...
| 5 — Deterministic downbeat | **Implemented (JS-estimator variant)** after device testing confirmed F1/F2/F3: capture rolls through the count-in, capture start is estimated from recorder duration updates (min-over-events, late-biased ≤ ~20 ms, offset by a 15 ms early-safety margin), the downbeat comes from `getGridAnchor()`, and the head is trimmed at save via `trimAudioRanges`. Overdubs trim to the guide's MEASURED start (anchored `currentTime`), and the guide is aimed at the downbeat instead of starting a beat early. Expected accuracy ±15–25 ms (vs ~50–500 ms before). The native recorder-timestamp patch (below) remains the path to ±5 ms and should still happen. |
