...
| Metronome blocks | `common/metronome/MetronomeBlocks.tsx` | The one metronome vocabulary, shared by the recorder sheet and the Metronome page (2026-09-16). `MeterRow`: the eight meters on ONE line, small tabular keys, active = tonal wash. `FeelControl` ("Click on"): a glyph+word `SegmentedControl` of the feels players actually set for that meter (simple: beats/eighths/triplets/sixteenths · 5/4: 3+2/2+3/eighths · 7/8: its three groupings · compound: in two/three/four or eighths) + Custom, and the `BarStrip` under it — the bar as dots (filled accent, hollow click, small rest, ticks for sub-clicks), which becomes the editor in Custom (tap a pulse: accent → click → rest). Subdivision and grouping are never shown as separate questions. `CueRows`: Beep / Visual / Haptic as editorial ink toggles, each carrying its own settings on its row (Beep: volume slider + sound key; Haptic: strength key); a cue that is off keeps its row. Rest-based feels appear only with `supportsSilentBeats`. |
