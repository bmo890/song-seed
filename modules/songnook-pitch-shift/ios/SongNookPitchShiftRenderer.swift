...
final class SongNookPitchShiftRenderer {
  ...
  private func sanitizeFileName(_ value: String?) -> String {
    ...
    let fallback = "songnook-transform-\(UUID().uuidString)"
  ...
  func renderTrim(_ request: [String: Any]) throws -> [String: Any] {
    ...
    guard let rawRanges = request["ranges"] as? [Any] else {
      throw NSError(domain: "SongNookPitchShift", code: 21, userInfo: [NSLocalizedDescriptionKey: "Trim rendering requires ranges."])
    }
    let outputFileName = (request["outputFileName"] as? String)?.trimmingCharacters(in: .whitespacesAndNewlines)
    let inputURL = URL(string: inputUri) ?? URL(fileURLWithPath: inputUri)
    let asset = AVURLAsset(url: inputURL)
    guard let sourceTrack = asset.tracks(withMediaType: .audio).first else {
      throw NSError(domain: "SongNookPitchShift", code: 22, userInfo: [NSLocalizedDescriptionKey: "No audio track found."])
    }
    ...
    guard let compTrack = composition.addMutableTrack(
      ...
      throw NSError(domain: "SongNookPitchShift", code: 23, userInfo: [NSLocalizedDescriptionKey: "Could not create composition track."])
    }
    ...
    exportSession.exportAsynchronously {
      if exportSession.status == .failed {
        exportError = exportSession.error
          ?? NSError(domain: "SongNookPitchShift", code: 26, userInfo: [NSLocalizedDescriptionKey: "Trim export failed."])
      }
      semaphore.signal()
    }
    semaphore.wait()
  ...
  func computeWaveform(_ request: [String: Any]) throws -> [String: Any] {
    ...
    guard let inputUri = request["inputUri"] as? String else {
      throw NSError(domain: "SongNookPitchShift", code: 27, userInfo: [NSLocalizedDescriptionKey: "Waveform analysis requires inputUri."])
    }
    let numberOfPoints = max(1, (request["numberOfPoints"] as? Int) ?? Int((request["numberOfPoints"] as? Double) ?? 256))
    let inputURL = URL(string: inputUri) ?? URL(fileURLWithPath: inputUri)
    let asset = AVURLAsset(url: inputURL)
    guard let track = asset.tracks(withMediaType: .audio).first else {
      throw NSError(domain: "SongNookPitchShift", code: 28, userInfo: [NSLocalizedDescriptionKey: "No audio track found."])
    }
    ...
    if reader.status == .failed {
      throw reader.error
        ?? NSError(domain: "SongNookPitchShift", code: 29, userInfo: [NSLocalizedDescriptionKey: "Waveform analysis failed."])
    }
  ...
  func getAudioDurationMs(_ request: [String: Any]) throws -> [String: Any] {
    guard let inputUri = request["inputUri"] as? String else {
      throw NSError(domain: "SongNookPitchShift", code: 31, userInfo: [NSLocalizedDescriptionKey: "Duration probe requires inputUri."])
    }
    let inputURL = URL(string: inputUri) ?? URL(fileURLWithPath: inputUri)
    let asset = AVURLAsset(url: inputURL)
    guard asset.tracks(withMediaType: .audio).first != nil else {
      throw NSError(domain: "SongNookPitchShift", code: 32, userInfo: [NSLocalizedDescriptionKey: "No audio track found."])
    }
